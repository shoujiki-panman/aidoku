...
AI回答の実文はextractorから、検証済み点数はscorerが保存した4判定Evaluatorから読む。
...
- `found=true` は「AI回答が返った」という観測値
- `found=false` は「回答が返らなかった」という観測値
- **どちらも正解・不正解の証明ではない**
- 公開画面の回答文と4/4表示はこの観測を使う

従来は `found=true` をそのまま20点にしていた。内容を検証していない回答まで満点に
なるため廃止した。旧23区データの回答文は消さず、正解点だけ未検証とする。

### B. 4判定Evaluatorの点数

各fact_typeを次の4つで判定する。

| 判定 | 方法 |
|---|---|
| `answer_correct` | `found`がGround Truthの情報有無と一致するか |
| `evidence_exists` | Evidence Checkで引用全文が渡した本文に実在するか |
| `evidence_supports_answer` | 答えとEvidenceだけをLLMが比較する |
| `ground_truth_matches` | 人手で固定した必須要素を回答が満たすか |

状態は `pass` / `fail` / `not_checked` / `not_applicable` の4つ。

- 4判定の必要項目がすべて `pass` → 20点
- 1つでも `fail` → 0点
- 必要な判定に `not_checked` がある → `null`（未検証）
- 情報が無いことが正解なら、Evidenceの2判定は `not_applicable` にできる

未検証を0点へ落とすと「間違い」と「まだ確認していない」が混ざるため、数値にしない。
現在の公開69マスはGround Truthが揃っておらず、正解点は未検証。

### C. 採点器の部分点（`scorer/`）

人手のゴールデンセット（`scorer/golden/tennyu.csv`・**3自治体12行のみ**／練馬・江戸川・八王子）と
突き合わせ、必須要素の充足率を部分点として残す診断値。4判定の合否とは別に、
どの要素が欠けたかを見るために使う。
...
> **言い間違えやすいので固定する。**
> 「23区の点数のぶれは±2点」は**誤り。**23区は4判定自体が未完了で、正解点は未検証。
