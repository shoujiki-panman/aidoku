...
- 記録するのは**AIが送ってきた質問文**。集計画面は `DEMAND_TOKEN` で閉じてあるが、
...
| | いまの状態 |
|---|---|
| 集計画面 `/_aidoku/demand` | `Authorization: Bearer <DEMAND_TOKEN>` が要る。**未設定なら閉じたまま**（404）。ただし**レート制限はまだ無い** |
...
`gennai_app/server.py` の `/invoke` は、入力されたURLをそのまま取得層へ渡す。
検証が `^https?://` だけだと `http://169.254.169.254/...` のような内部宛先を
こちらに取りに行かせられるので、`crawler/url_guard.py` で塞いでいる。

| | |
|---|---|
| scheme | `http` / `https` 以外は取りに行かない |
| 名前解決 | **できなければ拒否**（分からないものは通さない） |
| アドレス | 解決結果に private / loopback / link-local / multicast / 予約 が**1つでも**あれば拒否（DNS rebinding 対策で全部見る） |
| IPv4射影 | `::ffff:127.0.0.1` のような形でも中身で判定する |
| リダイレクト | urllib は 302 を黙って追うので、`GuardedRedirectHandler` が**1ホップずつ**同じ判定を通す |
| 本文サイズ | 8MB を超えたら読まずに捨てる |

robots.txt の fail-closed が偶然この一部を塞いでいたが、**robots.txt が 404 を返す
内部エンドポイントには通ってしまう**（404＝robotsが無い＝全許可に倒れる仕様のため）。
だから明示的に塞いでいる。テストは `crawler/test_url_guard.py`（17件）と
`crawler/test_polite_fetch.py` の `SsrfGuard`（5件）。
