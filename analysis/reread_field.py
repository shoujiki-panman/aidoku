...
# 手続きの説明。★ここに **転入届の説明を全手続きに使っていた。**
#   「粗大ごみ収集の申込とは、他の市区町村から引っ越してきたときに出す届出です」と
#   AIに教えていたので、実測（墨田区）で AI は粗大ごみ処理手数料一覧表を正しく読んだ上で
#   「転入時の届出の手数料ではない」と却下した。**読めても、聞き方が違えば取れない。**
PROC_DESC = {
    "転入届": "他の市区町村から{muni}へ引っ越してきたときに出す届出です。",
    "児童手当の申請": "中学校修了前（一部は高校生年代）の子どもを養育する人が"
                "{muni}に出す、児童手当の認定請求です。",
    "粗大ごみ収集の申込": "家庭から出る粗大ごみを{muni}に出すときの、収集の申込です。"
                  "品目ごとの処理手数料（処理券・シールの料金）も、この手続きの手数料です。",
}

# その項目として数えないもの。**手続きごとに違う。**
NOT_THIS = {
    "転入届": {
...
- **転出届・転居届**など、別の届出の手数料""",
    ...
    "児童手当の申請": {
        "必要書類": """- **児童扶養手当・特別児童扶養手当**など、別の手当の必要書類
- 保育園の入園申込・医療費助成（マル乳・マル子）の必要書類
- 「様式ダウンロード」のリンク名だけで、何を持参するかが本文に無い場合""",
        "期限": """- **支給日**（振込日）や現況届の提出時期だけの案内
- 手当の**支給対象年齢**（0歳〜高校生年代など、期限ではない）
- 受付時間・開庁時間（これは「窓口」の話）""",
        "窓口/オンライン可否": """- 児童手当以外の手当・助成の窓口
- 区の代表電話・問い合わせフォームだけの案内
- 電子申請の一般的な案内で、児童手当が対象と読めないもの""",
        "手数料": """- **支給額**（月額15,000円など）。これは手当の額であって手数料ではない
- 添付書類（課税証明など）を**他所で取るとき**の交付手数料
- Adobe Reader などソフトの「無料ダウンロード」""",
    },
    "粗大ごみ収集の申込": {
        "必要書類": """- 事業系ごみ・引越し業者への依頼など、**区の収集ではない**話
- 家電リサイクル法の対象（テレビ・冷蔵庫等）だけの案内
- 「様式ダウンロード」のリンク名だけで、何が要るかが本文に無い場合""",
        "期限": """- 収集日そのもの（曜日・カレンダー）だけの案内
- 受付時間・開庁時間（これは「窓口」の話）
- 「◯月◯日まで」のイベントやキャンペーンの期日""",
        "窓口/オンライン可否": """- 資源・可燃ごみなど、粗大ごみ以外の出し方
- 区の代表電話・問い合わせフォームだけの案内
- リユース事業者（ジモティー等）への出品案内""",
        "手数料": """- 家電リサイクル料金（テレビ・冷蔵庫等。区の処理手数料ではない）
- 事業系ごみの処理費用
- Adobe Reader などソフトの「無料ダウンロード」

★**品目ごとの料金表（処理券・シールの枚数や金額）は、この手続きの手数料です。**
  「一覧表だから手数料の記載ではない」と却下しないでください。""",
    },
}


def proc_rules(proc: str, field: str, muni: str) -> tuple[str, str]:
    """手続きの説明と、その項目として数えないもの。

    ★知らない手続きは黙って通さない。転入届の説明を使い回して、
      実測で判定を1件落としている。
    """
    if proc not in PROC_DESC or proc not in NOT_THIS:
        raise SystemExit(f"手続きの説明が無い: {proc}（reread_field.PROC_DESC に足す）")
    return PROC_DESC[proc].format(muni=muni), NOT_THIS[proc][field]
...
def ask_page(target: dict, muni: str, proc: str, field: str, model: str) -> dict:
    proc_desc, not_this = proc_rules(proc, field, muni)
    prompt = ASK.format(muni=muni, proc=proc, field=field, proc_desc=proc_desc,
                        not_this=not_this,
