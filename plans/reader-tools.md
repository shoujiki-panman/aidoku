...
- 2026-08-23 効果をキャッシュだけで測定（LLM 0回）。27セルで手続き名つきリンク110件が新たに渡る、逆に失うもの0。`analysis/probes/check_link_order.py`
...
- 2026-08-23 効果をキャッシュだけで測定（LLM 0回）。**表の中にしかない項目があるのは4セル・6項目。見積もりの5セルではなかった**（`analysis/probes/check_tables.py`）。28セルで見出しつきの表 28,946字が新たに渡る、本文が削れたセルは0。
