...
| **見張り（安い確認）** | ✅ `python3 crawler/check_pages.py` で「前回から変わったか」だけを条件付きGETで確認し `web/data/site-status.json` に書く。**LLMを呼ばない。**実測: 23区で**1分5秒・21区が304**（変化なし）。初回だけ `--prime` で土台作り（1分12秒）。⚠️ 渋谷区・品川区はサーバーがヘッダを返さず判定不可 |
