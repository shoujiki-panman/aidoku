...
| **定期確認** | ✅ GitHub Actions `check-pages.yml` が**毎朝JST 8:00**に確認。変わったときだけ Issue（ラベル`自動検知`）を立てる。何も変わらない日は通知しない。結果は `web/data/site-status.json` にコミットされ、**公開画面の最上部に出る**（普段は静かな1行、消えた・変わったときだけ大きくなる）。**404/410 は「消えた」として別扱い** |
