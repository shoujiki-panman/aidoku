...
## 実測で確かめたこと（2026-09-09・本番で本物のChatGPT）

個人枠の workers.dev に門番をデプロイし、本物の ChatGPT（GPT系のブラウズ）に
サイトを読ませて観測した（`plans/gatekeeper-direction.md` の「招待を待たずに観測する手順」）:

- **ChatGPT は本当に Web Bot Auth の署名を付けて来る。** ページ本体だけでなく
  JS・JSONのサブリソースの取得にも全部付けてくる（1訪問で9リクエストを記録）
- 実測した署名の形: `sig1=("@authority" "@method" "signature-agent");created=…;keyid="…";alg="ed25519";…;tag="web-bot-auth"`、
  `signature-agent: "https://chatgpt.com"`、UA は curl 系
- **こちらの検証器は `@authority` しか派生コンポーネントに対応しておらず、
  `@method` 入りの実物は base-construction-failed で全件 unverified になっていた。**
  → `@method` / `@path` / `@scheme` / `@target-uri` に対応（`test_derived.mjs`）。
  「観測してから決める。想像で作らない」がそのまま当たった形
- ChatGPT は読んだ結果を、設計どおりの言い方で利用者に返した:
  「このページだけでは持ち物・期限・費用・オンライン可否は**確定できません**」
  「**世田谷区が情報を公開していない、とまでは言えません**（到達未確認と明記されている）」
  ＝ 実測の穴と未確認の区別が、本物のAIの回答にそのまま乗った
...
## アクセス集計の注意（2026-09-12）

`answered` はAIの最終回答の成否ではなく、窓口の対応情報の有無を表す互換フィールドです。
質問のないHTTPアクセス、または `?q=` の項目を特定できない場合は `null`（判定対象外）にします。
`totals.undetermined` にはこの判定対象外も含み、聞き返しの件数とは限りません。
署名検証に失敗したアクセスは別集計で、署名なしが人間かAIかは判別しません。

過去の質問なしレコードも集計時に補正します。保存済みの公開集計は `normalizeDemandSnapshot` で訂正でき、元の件数・時刻は保持します。
エクスポート時はJSON・CSV・要約を同じ集計から生成します。画面や本文の情報欠落、住民の手続完了はこの集計だけでは判断できません。
